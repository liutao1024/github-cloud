...
public class RSAUtil {
    ...
    public static void generateKeyToFile(String algorithm, String pubPath, String priPath) throws Exception {
        ...
        // 进行Base64编码
        String publicKeyString = Base64Util.encode(publicKeyEncoded);
        String privateKeyString = Base64Util.encode(privateKeyEncoded);
        // 保存文件
        FileTool.writeFileByLines(new File(pubPath), publicKeyString);//(new File(pubPath), publicKeyString, Charset.forName("UTF-8"));
        FileTool.writeFileByLines(new File(priPath), privateKeyString);//, Charset.forName("UTF-8"));
