 ...
 * @ClassName: BatchJobGroup10001 
 * @Description: TODO(Describe)
 */
@SuppressWarnings("rawtypes")
public class TaskJobGroup implements Job{
	@Autowired
	private SystemBatchTaskDispathControlService systemBatchTaskDispathControlImpl = SpringContextUtil.getBean(SystemBatchTaskDispathControlService.class);
	@Autowired
	private SystemBatchTimeTaskDispathResultService systemBatchTimeTaskDispathResultServiceImpl = SpringContextUtil.getBean(SystemBatchTimeTaskDispathResultService.class);
	@Override
...
// 	        String theDayAfterTomorrow = CommUtil.toGetDateStrByDateStr(newToDay,2);
		...
		String TRIGGER_JOB_GROUP_NUMBER = (String) jobDataMap.get("triggerGroupNumber");//"10001";
...
//		System.out.println(systemBatchTaskDispathControlList.size());
		for (SystemBatchTaskDispathControl systemBatchTaskDispathControl : systemBatchTaskDispathControlList) {
			String status = "";
			String message = "";
			String jobClassName = systemBatchTaskDispathControl.getJobPath() + CommUtil.DOT + systemBatchTaskDispathControl.getJobModule() + CommUtil.JOBIMPL + systemBatchTaskDispathControl.getJobClass();
			String jobMethodName = systemBatchTaskDispathControl.getJobMethod();
			//同样通过发射找到类调用对应的方法
			Class[] classes = {};
			Object[] objects = {};
			try {
				...
				try {
					entity.setTriggerJobGroupNumber(TRIGGER_JOB_GROUP_NUMBER);
