...
//			this.parent = parent;
	 ...
	 * @return
	 */
	public RBNode<T, D> max(RBNode<T, D> node) {
		// 一直往左走,最左端的就是最小值,这是二叉树的性质
	...
	/**
	 * 寻找待删节点的后继节点 (因为这个节点即将要被删了,所以要选个后继节点补到这个位置来, 选择节点的规则:
	...
	/**
	 * 对某个节点进行左旋 (当前节点就是父亲节点,整体过程就是 父亲下沉,右孩子上升,然后右孩子的左节点变成了原父亲的右节点)
	 * @param x
	 */
		...
		while (((parent = parentOf(node)) != null) && isRed(parent)) {
			...
			if (gparent.leftChild == parent) {
				...
				if (isRed(uncle)) {
					...
					// 这里为什么祖父还要右旋一次呢？
					/*
					 * 黑祖 黑祖 / \ / \ 红父 黑叔 --> 红插 黑叔 \ / 红插 红父
					 * 可以看到上面,经过了一次旋转之后,平衡性仍然满足,但是颜色就不对了,仍然存在双红情况
					 * 其实理解了AVL树的平衡调整,这里其实也是一个“双旋转”的过程,只有被删节点与父亲不同侧时才需要双旋
					 * 继续接着上面的双红情况,因为颜色不对,这时需要变色 黑祖 红祖 / \ / \ 红插 黑叔 --> 黑插 黑叔 /
					 * / 红父 红父
					 * 可以看到,变色之后,右边的子树和左边子树的黑色节点个数保持一致,明显和原来不同,原来的图,看左上角,
					 * 右子树的黑色节点要多一个
					 * 原来的树肯定是满足红黑树性质的,所以这里只需要做一次简单的右旋,就可以让右子树的黑色节点个数再次比左边多一个
					 * ,满足了原来的情况
					 */
					rightRonate(gparent);
				}
	...
	/**
	 * @Author LiuTao @Date 2020年11月11日 下午5:35:48 
	 * @Title: printTreeLevel2 
